...
use tui::widgets::{
    Axis, BarChart, Block, Borders, Chart, Dataset, Gauge, List, ListState, Marker, Paragraph, Row,
    Sparkline, Table, Tabs, Text,
};
use tui::Frame;
...
impl UI {
    pub fn draw<B: Backend>(&mut self, f: &mut Frame<B>, app: &App) {
        ...
        match app.tabs.index {
            0 => self.draw_first_tab(f, &app, chunks[1]),
            1 => self.draw_second_tab(f, &app, chunks[1]),
            _ => {}
        };
    }

    fn draw_first_tab<B>(&mut self, f: &mut Frame<B>, app: &App, area: Rect)
    where
        B: Backend,
    {
        let chunks = Layout::default()
            ...
            .split(area);
        self.draw_gauges(f, app, chunks[0]);
        self.draw_charts(f, app, chunks[1]);
        self.draw_text(f, chunks[2]);
    }

    fn draw_gauges<B>(&mut self, f: &mut Frame<B>, app: &App, area: Rect)
    where
        B: Backend,
    {
        let chunks = Layout::default()
    ...
    fn draw_charts<B>(&mut self, f: &mut Frame<B>, app: &App, area: Rect)
    where
        B: Backend,
    {
            ...
            .split(area);
        {
                ...
                .split(chunks[0]);
            {
                ...
                let tasks = List::new(tasks)
                    ...
                    .highlight_symbol(">");
                f.render_stateful_widget(tasks, chunks[0], &mut self.tasks);
                ...
                let events =
                    List::new(events).block(Block::default().borders(Borders::ALL).title("List"));
                f.render_stateful_widget(events, chunks[1], &mut self.events);
            }
    ...
    fn draw_text<B>(&mut self, f: &mut Frame<B>, area: Rect)
    where
        B: Backend,
    {
        let text = [
    ...
    fn draw_second_tab<B>(&mut self, f: &mut Frame<B>, app: &App, area: Rect)
    where
        B: Backend,
    {
        let chunks = Layout::default()
        ...
        let table = Table::new(header.into_iter(), rows)
